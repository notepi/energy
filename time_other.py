...
if __name__ == "__main__":
    ...
    timeSequence=timeSequence.div(10000).reset_index(drop=True)
    
    del data[u"时间"]
    del data[u"第一产业增加值(亿元)"]
    del data[u"人均国内生产总值(元)"]
    
    otherdata=data.iloc[length-1:,].div(10000).reset_index(drop=True)
    timeSequence=pd.concat([otherdata,timeSequence],axis=1)
    # 生成训练数据和测试数据
